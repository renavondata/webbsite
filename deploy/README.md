...
every page accepts, which is ~1100 requests and the slow part of the run; set
`ROUTE_CHECK_SORTS=0` to skip it while debugging something else), and every `live` check in `checks.txt`
