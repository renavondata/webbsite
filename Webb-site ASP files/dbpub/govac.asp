...
Dim title,x,y,arrA,periods,numper,res,numh,i,i1,graphTitle,where,head,bread,parentID,firstd,neg,t,rev,links,_
	total,totals,line,useline,origtxt,app,h3,g,gval,yTitle,yround,con,rs
...
rs.Open "SELECT IFNULL(a.parentID,g.parentID)p,IFNULL(a.txt,g.txt)txt,g.txt origtxt,firstd,head,rev,approved,h3 FROM "&_
	"govitems g LEFT JOIN govadopt a ON g.ID=a.govitem AND tree="&t&" WHERE ID="&i,con
	parentID=rs("p")
	title=rs("txt")
...
If rs.EOF Then
	...
	arrA=con.Execute("SELECT ID,IFNULL(a.txt,g.txt),head,IFNULL(IFNULL(a.short,a.txt),IFNULL(g.short,g.txt)) FROM "&_
		"govitems g LEFT JOIN govadopt a ON ID=govitem AND tree="&t&" WHERE ID="&i).getRows
	'arrA=con.Execute("SELECT ID,txt,head,IFNULL(short,txt) FROM govitems WHERE ID="&i).getRows
Else
...
Do Until isNull(parentID)
	rs.Open "SELECT IFNULL(a.parentID,g.parentID)p,IFNULL(a.txt,g.txt)txt FROM govitems g LEFT JOIN govadopt a "&_
		"ON g.ID=a.govitem AND tree="&t&" WHERE ID="&parentID,con
...
If g Then
	...
	yround=3
	Dim gdp
	gdp=con.Execute("SELECT d,act FROM govac WHERE govitem=6060 and d>='" & periods(0) & "' ORDER BY d").GetRows()
	For y=0 to numh+1
		For x=0 to numPer
...
<h2><%=bread%></h2>
<%Call govacBar(1)%>
<form method="get" action="govac.asp">
	...
	<table class="numtable center">
		...
		<tr>
			...
			<%If links And arrA(0,y)<>i Then%>
				<a href="govac.asp?t=<%=t%>&amp;g=<%=g%>&amp;i=<%=arrA(0,y)%>"><%=arrA(1,y)%></a>
			<%Else%>
				<%=arrA(1,y)%>
			<%End If%>
			</td>
