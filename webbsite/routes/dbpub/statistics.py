...
def enigma_orgdata():
    """
    Spurious migration-artifact route (there is no original enigma.orgdata.asp);
    the real, fully-featured page is orgdata.asp. Redirect there, preserving
    the query string, rather than render this duplicate's mismatched output.
    """
    from flask import redirect

    _qs = request.query_string.decode()
    return redirect("/dbpub/orgdata.asp" + (("?" + _qs) if _qs else ""), code=301)
